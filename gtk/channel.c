...
struct _Channel
{
	...
	int gain_max;
	int gain_gap_end;
	double gain_last_value;
	bool correcting_gain;
	Channel *right;
...
static void
on_gain_value_changed(GtkAdjustment *adjustment, gpointer ptr)
{
	Channel *channel;
	double value, corrected;

	channel = OSCMIX_CHANNEL(ptr);
	if (channel->correcting_gain)
		return;
	value = gtk_adjustment_get_value(adjustment);
	if (channel->gain_gap_end > 0 && gtk_adjustment_get_lower(adjustment) == 0 &&
	    value > 0 && value < channel->gain_gap_end) {
		corrected = channel->gain_last_value <= 0 ? channel->gain_gap_end : 0;
		channel->correcting_gain = true;
		gtk_adjustment_set_value(adjustment, corrected);
		channel->correcting_gain = false;
		value = corrected;
	}
	channel->gain_last_value = value;
}

static void
osc_gainrange(GValue *arg, guint len, gpointer ptr)
{
	Channel *channel;
	int lower, upper;
	double step;
	...
	if (len < 2 || !G_VALUE_HOLDS_INT(&arg[0]) || !G_VALUE_HOLDS_INT(&arg[1]))
		return;
	channel = OSCMIX_CHANNEL(ptr);
	lower = g_value_get_int(&arg[0]);
	upper = g_value_get_int(&arg[1]);
	if (lower < 0 || upper < lower)
		return;
	step = upper > 24 ? 1.0 : 0.5;
	mixer_set_adjustment_range(channel->ui.gain, lower, upper, step);
}
...
channel_constructed(GObject *obj)
{
	...
	case CHANNEL_TYPE_INPUT:
		...
		{
			mixer_set_adjustment_range(self->ui.gain, self->gain_min, self->gain_max,
			                           self->gain_max > 24 ? 1.0 : 0.5);
			g_signal_connect(self->ui.gain, "value-changed", G_CALLBACK(on_gain_value_changed), self);
			...
			mixer_connect(osc, g_strdup_printf("/%s/%d/gainrange", type->value_nick, self->id), osc_gainrange, self);
...
channel_set_property(GObject *obj, guint id, const GValue *val, GParamSpec *spec)
{
	...
	case PROP_ID:
		...
		break;
	case PROP_GAIN_MIN:
		self->gain_min = g_value_get_int(val);
		break;
	case PROP_GAIN_MAX:
		self->gain_max = g_value_get_int(val);
		break;
	case PROP_GAIN_GAP_END:
		self->gain_gap_end = g_value_get_int(val);
		break;
	case PROP_MIXER:
...
channel_get_property(GObject *obj, guint id, GValue *val, GParamSpec *pspec)
{
	...
	case PROP_ID:
		...
		break;
	case PROP_GAIN_MIN:
		g_value_set_int(val, self->gain_min);
		break;
	case PROP_GAIN_MAX:
		g_value_set_int(val, self->gain_max);
		break;
	case PROP_GAIN_GAP_END:
		g_value_set_int(val, self->gain_gap_end);
		break;
	case PROP_MIXER:
...
static gboolean
on_volume_button_press(GtkWidget *widget, GdkEventButton *event, gpointer ptr)
{
...
channel_class_init(ChannelClass *class)
{
	...
	gtk_widget_class_bind_template_callback(GTK_WIDGET_CLASS(class), on_volume_button_press);
	...
	g_object_class_install_property(G_OBJECT_CLASS(class), PROP_GAIN_MIN,
									g_param_spec_int("gain-min", NULL, NULL, -100, 100, 0, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
	g_object_class_install_property(G_OBJECT_CLASS(class), PROP_GAIN_MAX,
									g_param_spec_int("gain-max", NULL, NULL, 0, 100, 75, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
	g_object_class_install_property(G_OBJECT_CLASS(class), PROP_GAIN_GAP_END,
									g_param_spec_int("gain-gap-end", NULL, NULL, 0, 100, 0, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
