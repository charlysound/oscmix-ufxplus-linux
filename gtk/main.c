...
#define DEVICE_PROBE_INTERVAL_MS 500
...
struct _OSCMixWindow {
	...
	guint device_probe_source;            /* retries /device/info until detected */
	gpointer send_host;
	gpointer send_port;
	gpointer recv_host;
	gpointer recv_port;
...
static gboolean
probe_device(gpointer ptr)
{
	OSCMixWindow *self;

	self = OSCMIX_WINDOW(ptr);
	if (self->current_device) {
		self->device_probe_source = 0;
		return G_SOURCE_REMOVE;
	}
	mixer_send(self->osc, "/device/info", NULL);
...
address_resolved(GObject *obj, GAsyncResult *res, gpointer ptr)
{
	...
	/* Discover the device first. A second refresh is sent after all channel
	 * bindings exist, so no initial values can be lost. Keep the early refresh
	 * for compatibility with older backends which identify on /device. */
	probe_device(self);
	if (self->device_probe_source == 0)
		self->device_probe_source = g_timeout_add(DEVICE_PROBE_INTERVAL_MS, probe_device, self);
...
apply_device(OSCMixWindow *self, const struct device *dev)
{
	...
	if (self->device_probe_source != 0) {
		g_source_remove(self->device_probe_source);
		self->device_probe_source = 0;
	}
	...
	mixer_send(self->osc, "/device/channels", NULL);
	mixer_send(self->osc, "/device/names", NULL);
	mixer_send(self->osc, "/refresh", NULL);
}
...
/*
 * on_device_osc - called for the current /device/id response or the legacy
 * /device response. Matches the id and applies the device exactly once.
 */
static void
...
setup_channels(OSCMixWindow *self, const struct device *dev, ChannelType type, GtkBox *box)
{
	...
	for (i = 0; i < num_channels; ++i) {
		...
		chan = g_object_new(channel_get_type(),
							...
							"flags", chflags,
							"id", i + 1,
							"gain-min", channels[i].gain.min,
							"gain-max", channels[i].gain.max,
							"gain-gap-end", channels[i].gain.instrument_min,
							"name", name,
...
oscmix_window_init(OSCMixWindow *self)
{
	...
	/* Current backend identity plus the legacy combined identity response. */
	mixer_connect(self->osc, "/device/id", on_device_osc, self);
	mixer_connect(self->osc, "/device", on_device_osc, self);

	/* Start timeout - if no device identity arrives, show offline dialog. */
