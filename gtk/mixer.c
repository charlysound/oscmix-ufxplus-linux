...
void
mixer_set_adjustment_range(GtkAdjustment *adjustment, double lower, double upper, double step)
{
	double value;

	value = gtk_adjustment_get_value(adjustment);
	in_osc_to_prop = true;
	gtk_adjustment_configure(adjustment, CLAMP(value, lower, upper),
	                         lower, upper, step, step * 10, 0);
